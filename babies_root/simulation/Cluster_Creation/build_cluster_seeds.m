...
function [trace_cluster_seed,seed_distances] = build_cluster_seeds(base_name,run_name)
global SIMOPTS;
trace_cluster_seed = [];  seed_distances = [];
if (exist([make_data_name('trace_cluster_seed',base_name,run_name,0) '.mat'])~=2 || ...
    exist([make_data_name('seed_distances',base_name,run_name,0) '.mat'])~=2) || ...
    SIMOPTS.write_over==1, 
go = 1;
[p,go] = try_catch_load(['population_' base_name run_name],go,1); %get corresponding population
if go==1, 
if SIMOPTS.reproduction~=2, 
  [tx,go] = try_catch_load(['trace_x_' base_name run_name],go,1); %get corresponding trace_x
  if go==1,
    [ty,go] = try_catch_load(['trace_y_' base_name run_name],go,1);  %get corresponding trace_x
  end
end
if go==1, 
