...
% Determines trace_cluster_seed, num_clusters, trace_clusters, orgsncluster, centroids of 
% clusters and cluster_diversity given the necessary parameter options in SIMOPTS.
% The output gives the time
function [times] = Clustering()
global SIMOPTS;
...
for run = SIMOPTS.SIMS, 
  ...
  if SIMOPTS.reproduction==1 || exist([tcsn '.mat'])~=2, 
      [tcs] = build_trace_cluster_seed(base_name,run_name);  end
